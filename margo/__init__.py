...
def compile(text, context, mangle_names=False, file_hash=""):
    ...
    nr_ast = naming_rules.NamingRules(context.copy()).main(lp_ast)
    #print("Stage 3: checking name existence.")
    #ne_ast = name_existence.NameExistence(context.copy()).main(nr_ast)
    # print("Stage 4: doing type inference where needed.")
    # ti_ast = type_inference.TypeInference(context.copy()).main(ne_ast)
    # print("Stage 5: checking types.")
    # tc_ast = type_checking.TypeChecking(context.copy()).main(ti_ast)
    # print("Stage 6: adding default values where needed.")
    # dv_ast = default_value.DefaultValue(context.copy()).main(tc_ast)
    ...
    # arc_ast = arc.ARC(context.copy()).main(se_ast)
    print("Compiled!")
    return nr_ast
